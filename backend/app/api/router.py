...
async def share_calendar(campaign_id: str, format: str = "html", db: Session = Depends(get_db)):
    ...
    calendar = await content_calendar.get_content_calendar(campaign_id, days=30)
    posts = calendar.posts if hasattr(calendar, "posts") else []

    if format == "html":
        # Generate a clean HTML page the client can view
        html_lines = [
            "<!DOCTYPE html>",
            "<html><head>",
            f"<title>{campaign.name} — Content Calendar</title>",
            "<style>",
            "body{font-family:Inter,system-ui,sans-serif;max-width:800px;margin:40px auto;padding:0 20px;color:#333}",
            "h1{font-size:28px;margin-bottom:8px}",
            ".subtitle{color:#666;margin-bottom:32px}",
            ".post{border:1px solid #e5e5e5;border-radius:12px;padding:20px;margin-bottom:16px}",
            ".post-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}",
            ".platform{font-weight:600;color:#FF6B6B}",
            ...
            f"<div class='subtitle'>Content Calendar — Generated by Fankaar Digital</div>",
        ]

        for post in posts:
            status = getattr(post, "status", "scheduled")
            platform = getattr(post, "platform", "social")
            content = getattr(post, "content_text", "") or getattr(post, "content", "")
            scheduled = getattr(post, "scheduled_time", None)
            date_str = scheduled.strftime("%b %d, %Y at %I:%M %p") if scheduled else "TBD"

            html_lines.append("<div class='post'>")
            html_lines.append(f"<div class='post-header'><span class='platform'>{platform.title()}</span><span class='date'>{date_str}</span></div>")
            html_lines.append(f"<div class='content'>{content}</div>")
            html_lines.append(f"<span class='status {status}'>{status.title()}</span>")
            html_lines.append("</div>")

        html_lines.extend([
            "<div style='margin-top:40px;padding-top:20px;border-top:1px solid #e5e5e5;color:#999;font-size:12px'>",
            "Powered by Fankaar Digital — Tiny's Army AI Agency",
            "</div></body></html>",
        ])

        return {"format": "html", "html": "\n".join(html_lines), "post_count": len(posts)}

    elif format == "json":
        # Clean JSON the client can consume
        clean_posts = []
        for post in posts:
            clean_posts.append({
                "platform": getattr(post, "platform", ""),
                "content": getattr(post, "content_text", "") or getattr(post, "content", ""),
                "scheduled_time": getattr(post, "scheduled_time", None).isoformat() if getattr(post, "scheduled_time", None) else None,
                "status": getattr(post, "status", "scheduled"),
                "content_type": getattr(post, "content_type", "post"),
            })
        return {"format": "json", "campaign_name": campaign.name, "posts": clean_posts}
